...
class BertFineTuning():
    ...
    def log_results(self,target,cm):
        self.child_run.log('Overall ACC '+target,np.float(cm.overall_stat['Overall ACC']))
        self.child_run.log('F1 Macro '+target,np.float(cm.overall_stat['F1 Macro']))
        self.child_run.log('F1 Macro '+target,np.float(cm.overall_stat['F1 Micro']))
        self.child_run.log('Overall MCC '+target,np.float(cm.overall_stat['Overall MCC']))
        self.child_run.log('TPR Macro '+target,np.float(cm.overall_stat['TPR Macro']))
        self.child_run.log('TPR Micro '+target,np.float(cm.overall_stat['TPR Micro']))
        self.child_run.log('PPV Micro '+target,np.float(cm.overall_stat['PPV Micro']))
        self.child_run.log('PPV Macro '+target,np.float(cm.overall_stat['PPV Macro']))
            ...
            for e in range(self.last_epoch,self.epochs,1):
                ...
                with parent_run.child_run() as child:
                    ...
                    for i,(list_of_indices,segments_ids,labels) in enumerate(train_loader):
                        ...
                        self.child_run.log("epoch",np.float(e+1))
                        self.child_run.log("step",np.float((i+1)//self.print_every))
                        if((i+1)%self.print_every==0):
                            ...
                            self.child_run.log('batch_loss',np.float(batch_loss))
                            print('train results: \n')
                    ...
                    if(((e+1)>=self.validate_at_epoch)):
                        ...
                        self.child_run.log('test_loss',np.float(_loss))
